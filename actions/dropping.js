...
import { checkActivePoll, dropBook, getNominees } from '../database.js'

async function selectForDrop(req, res) {
  await res.send({
  ...
  return DiscordRequest(endpoint, {
    method: 'PATCH',
    body: {
      content: 'Select the book you wish to drop from the nominees',
      components: [
        {
          type: 1,
          components: [
...
async function dropNominatedBook(req, res) {
  const endpoint = `webhooks/${process.env.APP_ID}/${req.body.token}/messages/@original`
  ...
  try {
    ...
    return DiscordRequest(endpoint, {
      method: 'PATCH',
      body: {
        content: `Book dropped successfully  :white_check_mark:`,
        components: [],
        embeds: [],
      },
    })
...
export { dropNominatedBook, selectForDrop }
